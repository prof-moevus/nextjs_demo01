...
export default function ListeProduits() {
    ...
    const addItem = () => {
        setProducts((prevProducts) => {
            ...
            // const maxID = Math.max(...ids) //alternative hands-on de uuidv4
            // // console.log(maxID) 
            newProducts.splice(rd, 0, { id: uuidv4(), category: "NA", price: "-1$", stocked: false, name: "NA" })
            return newProducts
        })
    }
            ...
            <div className='flex flex-row flex-wrap h-1/2'>
                {products && products.map((item, i) => {
                    if (products.length < 8) {
                        return (item.stocked && (
                            <VignetteProduit item={item} key={item.id} listID={item.id} />
                        ))
                    } else {
                        return (!item.stocked && (
                            <VignetteProduit item={item} key={item.id} listID={item.id} />
